...
export default function PageHeader({ title, subtitle, ctaText, ctaTo, image }){
  ...
  return (
    <header className="relative bg-cover bg-center" style={{ backgroundImage: `linear-gradient(rgba(18,59,68,0.66), rgba(42,111,134,0.5)), url('${bg}')` }}>
      <div className="max-w-6xl mx-auto px-6 py-20 text-white">
        ...
        {ctaText && ctaTo && (
          <div className="mt-6">
            <Link to={ctaTo} className="inline-block bg-white text-[#123B44] px-4 py-2 rounded-lg font-semibold shadow">{ctaText}</Link>
          </div>
        )}
      </div>
    </header>
