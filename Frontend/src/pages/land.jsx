...
function getImageSrc(property) {
  // cover_image comes as "/uploads/filename.jpg" from the subquery
  if (property.cover_image) {
    return `${API_BASE}${property.cover_image}`;
  }
...
function PropertyCard({ property, onClick }) {
  ...
  // created_at field name — try both
  const dateStr = property.created_at || property.createdAt || null;

  return (
    <article
      onClick={onClick}
      className="overflow-hidden rounded-xl border bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg cursor-pointer"
    >
      <div className="relative">
        <img
          ...
          alt={property.title}
        />
        <span className="absolute left-3 top-3 bg-white/90 text-xs text-slate-800 px-2 py-1 rounded">
          For Sale
        </span>
        {property.land_area && (
          <span className="absolute right-3 top-3 bg-white/90 text-xs text-slate-800 px-2 py-1 rounded">
            {property.land_area} perches
          </span>
        )}
        {property.is_featured === 1 && (
          <span className="absolute left-3 bottom-10 bg-blue-700 text-white text-xs px-2 py-0.5 rounded">
            Featured
          </span>
        )}
        <div className="absolute left-3 bottom-3 bg-gradient-to-r from-black/60 to-transparent text-white px-3 py-1 rounded text-sm font-semibold">
      ...
      <div className="p-4">
        ...
        <p className="mt-1 text-sm text-slate-500 line-clamp-1">
          {[property.city, property.district_name || property.district].filter(Boolean).join(", ") || "Sri Lanka"}
        </p>
...
export default function Land() {
  ...
  const hasFilters = search || typeFilter || districtFilter || priceFilter || activeChip;
      ...
      <div className="bg-slate-50 py-8 px-4 sm:px-6 lg:px-8 font-sans">
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-center">
            <div className="md:col-span-4">
              <input
                type="text"
                placeholder="Search location, landmark or title"
                value={search}
            ...
            <div className="md:col-span-2">
              <select
                ...
                className="w-full rounded-md border border-slate-300 px-4 py-2 text-sm outline-none"
              >
            ...
            <div className="md:col-span-2">
              <select
                value={typeFilter}
                onChange={(e) => { setTypeFilter(e.target.value); setActiveChip(""); setPage(1); }}
                className="w-full rounded-md border border-slate-300 px-4 py-2 text-sm outline-none"
              >
            ...
            <div className="md:col-span-2 flex gap-2">
              <button
                type="submit"
                className="flex-1 rounded-md bg-blue-700 px-4 py-2 text-sm font-medium text-white transition hover:bg-blue-800"
              >
                Search
              </button>
              {hasFilters && (
          ...
          <div className="my-4 flex items-center gap-3 flex-wrap">
            {activeChip && (
              <span className="rounded-full border border-blue-700 bg-white px-4 py-1 text-sm">
                {activeChip} Lands
                <button onClick={() => { setActiveChip(""); setPage(1); }} className="ml-2 text-blue-700">✕</button>
              </span>
            )}
            {districtFilter && (
              <span className="rounded-full border border-blue-700 bg-white px-4 py-1 text-sm">
                {districtFilter}
                <button onClick={() => { setDistrictFilter(""); setPage(1); }} className="ml-2 text-blue-700">✕</button>
              </span>
            )}
            {typeFilter && (
              <span className="rounded-full border border-blue-700 bg-white px-4 py-1 text-sm">
                {typeFilter}
                <button onClick={() => { setTypeFilter(""); setPage(1); }} className="ml-2 text-blue-700">✕</button>
              </span>
            )}
            {priceFilter && (
              <span className="rounded-full border border-blue-700 bg-white px-4 py-1 text-sm">
                {priceFilter}
                <button onClick={() => { setPriceFilter(""); setPage(1); }} className="ml-2 text-blue-700">✕</button>
              </span>
            )}
            <button onClick={handleReset} className="ml-2 text-sm font-medium text-blue-700">
              RESET ALL
            </button>
          </div>
        )}
        ...
        <div className="mb-6 flex flex-wrap gap-3">
          {landTypes.map((c) => (
            <button
              key={c}
              onClick={() => { setActiveChip(activeChip === c ? "" : c); setTypeFilter(""); setPage(1); }}
              className={`rounded-full border px-4 py-1 text-sm transition ${
                activeChip === c
                  ? "bg-blue-700 text-white border-blue-700"
                  : "border-blue-700 bg-white text-slate-700 hover:bg-blue-50"
              }`}
            >
              {c} Lands
            </button>
                ...
                {totalPages > 1 && (
                  <div className="mt-8 flex items-center justify-center gap-2">
                    <button
                      ...
                      disabled={page === 1}
                      className="px-3 py-1.5 rounded-md border text-sm disabled:opacity-40 hover:bg-slate-100"
                    >
                      ← Prev
                    </button>
            ...
            <aside className="lg:sticky lg:top-24 space-y-5">
              ...
              <div className="rounded-xl border bg-white p-5">
                ...
                <p className="text-sm text-slate-600 mb-4">
                  Get a free market valuation or connect with a local agent.
                </p>
                <button
                  onClick={() => navigate("/find-agent")}
                  className="w-full bg-[#2171B5] hover:bg-[#1b5f98] text-white py-2 rounded-md font-medium text-sm"
                >
                  Contact Agent
                </button>
              </div>
              ...
              <div className="rounded-xl bg-white p-4 border">
                <div className="grid grid-cols-3 gap-3 text-center">
                  <div>
                    <div className="text-lg font-bold text-[#08306B]">
              ...
              <div className="rounded-xl border bg-white p-4">
                ...
                <p className="text-xs text-slate-500 mb-3">
                  Have land to sell? Reach thousands of buyers on LankaPropertyWeb.
                </p>
                <button
                  onClick={() => navigate("/post-ad")}
                  className="w-full rounded-md border border-blue-700 text-blue-700 py-2 text-sm font-medium hover:bg-blue-50 transition"
                >
                  + Post Free Ad
                </button>
              </div>
