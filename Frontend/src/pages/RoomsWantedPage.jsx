...
import { ChevronLeft, ChevronRight, Search, Bell, ShieldCheck } from 'lucide-react'

const roomListings = [
  { id: 790201, title: 'Room wanted near University of Peradeniya', area: 'Peradeniya, Kandy', summary: 'Looking for single room accommodation close to the university.', hint: 'Peradeniya / Kandy', badge: 'Student wanted', image: 'https://images.unsplash.com/photo-1505693416388-ac5ce068fe85?w=1400&q=80&auto=format&fit=crop' },
  { id: 790202, title: 'Room wanted in Colombo 10', area: 'Colombo 10, Maradana', summary: 'Seeking an affordable single room with shared kitchen near Maradana.', hint: 'Maradana', badge: 'Budget', image: 'https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=1400&q=80&auto=format&fit=crop' },
  { id: 790203, title: 'Room wanted in Kandy city', area: 'Kandy', summary: 'Female tenant looking for a single room in safe neighbourhood in Kandy.', hint: 'Kandy city', badge: 'Kandy wanted', image: 'https://images.unsplash.com/photo-1560448204-e02f11c3d0e2?w=1400&q=80&auto=format&fit=crop' },
  { id: 790204, title: 'Room wanted in Wattala', area: 'Wattala', summary: 'Looking for short-term single room close to transport links.', hint: 'Wattala', badge: 'Short term', image: 'https://images.unsplash.com/photo-1494526585095-c41746248156?w=1400&q=80&auto=format&fit=crop' },
]

const featuredProjects = ['Student Housing Kandy', 'Colombo Shared Living', 'Wattala Rooms']
const topCities = ['Colombo', 'Kandy', 'Wattala', 'Negombo']

function RoomsWantedPage() {
  const [query, setQuery] = useState('')
  const [locationQuery, setLocationQuery] = useState('')
  const [page, setPage] = useState(1)
  const [selectedListing, setSelectedListing] = useState(roomListings[0])
  ...
  const handleView = (l) => { setSelectedListing(l); document.getElementById('room-detail')?.scrollIntoView({ behavior: 'smooth' }) }
  const handleSave = (l) => setSavedIds((cur) => (cur.includes(l.id) ? cur.filter((id) => id !== l.id) : [...cur, l.id]))
  const handleContact = (l) => { window.location.href = `mailto:info@lankapropertyweb.com?subject=${encodeURIComponent('Room wanted: ' + l.title)}` }
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#08306B] via-[#2171B5] to-[#0d4a9f] text-white">
        <div className="relative mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
          <p className="text-xs font-bold uppercase tracking-wider text-blue-100">🛏️ Rooms wanted</p>
          <h1 className="mt-3 text-4xl font-black leading-tight sm:text-5xl">List of Rooms wanted</h1>
          <p className="mt-4 max-w-2xl text-base leading-relaxed text-white/80 sm:text-lg">Browse room wanted requests across Sri Lanka — students, professionals and short-term tenants included.</p>
        </div>
      </section>
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm lg:p-8">
          <div className="grid gap-4 lg:grid-cols-[1fr_320px] lg:items-end">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#2171B5]">🔍 Search rooms wanted</p>
              <h2 className="mt-2 text-2xl font-black text-slate-900">Search and filter room wanted ads</h2>
            </div>
          ...
          <div className="mt-6 flex flex-wrap gap-2 text-xs font-semibold text-slate-600 sm:text-sm">
            {['Short term', 'Student', 'Long term', 'Furnished'].map((c) => <span key={c} className="rounded-full bg-slate-100 px-4 py-2">{c}</span>)}
          </div>
        </section>
        ...
        <section className="mt-8 grid gap-6 lg:grid-cols-[280px_minmax(0,1fr)]">
          <aside className="h-fit rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <h3 className="mt-2 text-xl font-black text-slate-900">Popular rooms locations</h3>
            ...
            <div className="mt-6 rounded-2xl bg-[#f7fbff] p-4 ring-1 ring-[#d9e8f6]">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-600">Get instant alerts when room requirements match your listing.</p>
            </div>
          </aside>
          ...
          <div>
            <div className="mb-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#2171B5]">Rooms wanted</p>
                ...
                <article key={l.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm lg:grid lg:grid-cols-[220px_minmax(0,1fr)]">
                  <div className="relative h-44 lg:h-full">
                    ...
                    <div className="absolute left-4 top-4">
                      <span className="rounded-full bg-white/95 px-3 py-1 text-xs font-bold uppercase tracking-[0.16em] text-[#08306B]">Room</span>
                    </div>
                  </div>
            ...
            <div id="room-detail" className="mt-8 rounded-2xl border border-[#d9e8f6] bg-[#f7fbff] p-6">
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#2171B5]">Selected room wanted</p>
...
export default RoomsWantedPage
