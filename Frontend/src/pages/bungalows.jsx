import React from "react";
import Navbar from "../components/Navbar";
import { useNavigate } from "react-router-dom";

export default function Bungalows() {
  const navigate = useNavigate();

  return (
    <>
      <Navbar />

      <div className="bg-[#EAEAEA] p-5 font-sans text-[#4A4A4A]">
        
        {/* SEARCH BAR */}
        <div className="flex flex-wrap items-center gap-3 rounded-lg border border-[#B8B8B8] bg-white p-4">
          <input
            type="text"
            placeholder="School or University"
            className="rounded-md border border-[#B8B8B8] px-4 py-2 text-sm outline-none focus:ring-2 focus:ring-[#2A6FA3]"
          />

          {["Select Radius", "House", "Price Range", "Show More"].map((opt, i) => (
            <select
              key={i}
              className="rounded-md border border-[#B8B8B8] px-4 py-2 text-sm outline-none"
            >
              <option>{opt}</option>
            </select>
          ))}

          <button className="rounded-md bg-[#203F52] px-5 py-2 text-sm font-medium text-white transition hover:bg-[#2D4F63]">
            Search
          </button>
        </div>

        {/* FILTERS */}
        <div className="my-4 flex items-center gap-3">
          <span className="rounded-full border border-[#2A6FA3] bg-white px-4 py-1 text-sm">
            House ✕
          </span>

          <span className="cursor-pointer text-sm font-medium text-[#2A6FA3]">
            RESET ALL
          </span>
        </div>

        {/* TITLE */}
        <div className="mb-4 text-2xl font-bold text-[#203F52]">
          Bungalows for rent in Sri Lanka (1183 properties)
        </div>

        {/* ACTION BUTTONS */}
        <div className="mb-5 flex flex-wrap gap-3">
          {["Save Search", "Create E-Mail Alerts", "Share"].map((btn) => (
            <button
              key={btn}
              className="rounded-md border border-[#2A6FA3] bg-white px-4 py-2 text-sm transition hover:bg-[#EAEAEA]"
            >
              {btn}
            </button>
          ))}
        </div>

        {/* CHIPS */}
        <div className="mb-6 flex flex-wrap gap-3">
          {["Apartments", "Commercial", "Villas", "Rooms"].map((chip) => (
            <div
              key={chip}
              className="rounded-full border border-[#B8B8B8] bg-white px-4 py-1 text-sm"
            >
              {chip}
            </div>
          ))}
        </div>

        {/* CONTENT */}
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-4">
          
          {/* LEFT */}
          <div className="lg:col-span-3">
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-3">

              {[
                {
                  title: "Beautiful House",
                  img: "https://images.unsplash.com/photo-1568605114967-8130f3a36994",
                  loc: "Peradeniya, Kandy"
                },
                {
                  title: "Modern Villa",
                  img: "https://images.unsplash.com/photo-1507149833265-60c372daea22",
                  loc: "Katugastota, Kandy",
                  badge: "URGENT"
                },
                {
                  title: "Luxury Home",
                  img: "https://images.unsplash.com/photo-1494526585095-c41746248156",
                  loc: "Hanthana, Kandy",
                  badge: "URGENT"
                },
                {
                  title: "Stylish Apartment",
                  img: "https://images.unsplash.com/photo-1600607687939-ce8a6c25118c",
                  loc: "Ampitiya, Kandy",
                  badge: "URGENT"
                },
                {
                  title: "Family House",
                  img: "https://images.unsplash.com/photo-1570129477492-45c003edd2be",
                  loc: "Digana, Kandy",
                  badge: "URGENT"
                },
                {
                  title: "Modern Residence",
                  img: "https://images.unsplash.com/photo-1501183638710-841dd1904471",
                  loc: "Kandy City, Kandy",
                  badge: "URGENT"
                }
              ].map((item, i) => (
                <div
                  key={i}
                  className="overflow-hidden rounded-xl border border-[#B8B8B8] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg"
                >
                  <div className="relative">
                    {item.badge && (
                      <span className="absolute right-3 top-3 rounded bg-[#2A6FA3] px-2 py-1 text-[11px] font-semibold text-white">
                        {item.badge}
                      </span>
                    )}

                    <img
                      src={item.img}
                      alt={item.title}
                      onClick={() => navigate("/propertynew")}
                      className="h-[200px] w-full object-cover cursor-pointer"
                    />
                  </div>

                  <div className="p-4">
                    <h3 className="text-lg font-semibold text-[#203F52]">
                      {item.title}
                    </h3>
                    <p className="mt-1 text-sm text-[#4A4A4A]">
                      {item.loc}
                    </p>
                  </div>
                </div>
              ))}
            </div>
          </div>

          {/* RIGHT */}
          <div className="flex flex-col gap-5">

            <div className="rounded-xl bg-[#203F52] p-6 text-center text-white">
              <h3 className="text-xl font-bold">Market Report 2026</h3>
              <p className="mt-2 text-[#EAEAEA]">OUT NOW!</p>
            </div>

            <div className="rounded-xl border border-[#B8B8B8] bg-white p-4">
              <h4 className="mb-4 text-lg font-semibold text-[#203F52]">
                Featured Projects
              </h4>

              <img
                src="https://images.unsplash.com/photo-1613977257592-4871e5fcd7c4"
                alt="featured"
                className="rounded-lg"
              />
            </div>

          </div>
        </div>
      </div>
    </>
  );
}