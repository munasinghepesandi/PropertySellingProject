...
router.get('/properties', async (req, res) => {
  try {
    ...
    if (q)            { where += ' AND (p.title LIKE ? OR p.address LIKE ?)'; params.push(`%${q}%`,`%${q}%`); }
...
router.post('/properties/add', async (req, res) => {
  try {
    const { title, type, listing_type, district_id, city, address, price, price_type,
            bedrooms, bathrooms, floor_area, land_area, land_unit,
            status, description, is_featured, is_urgent, agent_name, agent_phone } = req.body;
    await db.query(
      `INSERT INTO properties
         (title, type, listing_type, district_id, city, address, price, price_type,
          bedrooms, bathrooms, floor_area, land_area, land_unit,
          status, description, is_featured, is_urgent, agent_name, agent_phone, created_at)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW())`,
      [title, type, listing_type, district_id||null, city, address, price||0, price_type||'total',
       bedrooms||0, bathrooms||0, floor_area||null, land_area||null, land_unit||'sqft',
       status||'pending', description, is_featured?1:0, is_urgent?1:0, agent_name, agent_phone]
    );
...
router.post('/properties/:id/edit', async (req, res) => {
  try {
    const { title, type, listing_type, district_id, city, address, price, price_type,
            bedrooms, bathrooms, floor_area, land_area, land_unit,
            status, description, is_featured, is_urgent, agent_name, agent_phone } = req.body;
    await db.query(
      `UPDATE properties SET
         ...
         land_area=?, land_unit=?, status=?, description=?,
         is_featured=?, is_urgent=?, agent_name=?, agent_phone=?
       WHERE id=?`,
      [title, type, listing_type, district_id||null, city, address, price||0, price_type||'total',
       bedrooms||0, bathrooms||0, floor_area||null, land_area||null, land_unit||'sqft',
       status||'pending', description, is_featured?1:0, is_urgent?1:0,
       agent_name, agent_phone, req.params.id]
    );
    res.redirect('/admin/properties');
...
router.post('/properties/:id/delete', async (req, res) => {
  try { await db.query('DELETE FROM properties WHERE id=?', [req.params.id]); }
...
router.get('/agents', async (req, res) => {
  try {
    ...
    // Get agents with property count via agent_name match (since properties has agent_name not agent_id)
...
// ← module.exports is LAST — all routes must be above this line
